...
Take the `rewrite` set, group by `dest_collection`, and slice into batches of **10**. Each batch becomes one PR. One collection per PR — never mix hacks and tools in the same review.
...
front-matter template + voice profile (`_data/brand/voice.yml`), the glossary (`_data/brand/glossary.yml`), the reused preview image path, and the quest links to preserve. It writes the rewritten file to the correct collection dir and returns a summary. Rules it must follow:
...
```
PR body: the collection, the 10 slugs, what was tested (harness verdict), which preview images were reused, and which quest links were preserved. Label `auto:content`
...
---
```
Hack (`pages/_hacks/<slug>.md`): `title, description, date, collection: hacks, author, excerpt, tags[]`, plus `preview:` when a hero image applies. Tool (`pages/_tools/<slug>.md`): the hack keys plus `collection: tools` and a non-empty `verdict:`.

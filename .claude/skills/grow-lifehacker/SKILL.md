...
PR, run the article through the preview-image generator so it publishes with cover art (the post card, the `og:image`, and the article banner all render from it):
  ...
  ```
  The wrapper resolves the **capability ladder** (`preview_images.provider: auto`): a raster API key → that AI renderer (Claude writes the art brief); a Claude credential alone (CI runs have one) → **Claude authors the banner as a content-aware SVG** via `scripts/claude_svg_banner.py`; nothing wired → the deterministic offline `local` SVG, so the step always runs for any fleet agent. Either way it writes the image into `assets/images/previews/` and stamps the `preview:` line into your front matter. Commit BOTH the generated image and the stamped front matter with the article — an article without its banner is not finished. (See /docs/the-plugin-that-isnt-a-plugin/.)
